...
select  count(id), month(date_published)  from movie group by date_published order by  date_published limit 10 ;
...
select count(id) , country from movie where country='india' or country='usa' and year=2019 group by country; -- (usa, india) ;

select count(id) , country from movie where country in ('usa','india') and year=2019 group by country; -- (usa, india) ;
select count(id) from movie where country='india'; -- 1007
select count(id) from movie where country='usa'; -- 2260
...
select genre.genre , avg(movie.duration) as avg_dur  from movie join genre on movie.id = genre.movie_id group by genre order by avg_dur ;
...
WITH genre_rank AS(
select genre.genre , count(movie.id) as Movie_counts,
RANK () OVER (ORDER BY movie.id ) as Rank_no
  from movie join genre on movie.id = genre.movie_id group by genre )
...
select r.median_rating, count(m.id) as movie_count from movie as m join
		ratings as r on m.id=r.movie_id group by r.median_rating order by r.median_rating ;
...
dense_rank () over(order by COUNT(id) desc)  as prod_company_rank
  from movie as m
join ratings as r on m.id = r.movie_id where avg_rating > 8  AND production_company IS NOT NULL
...
select  r.median_rating, COUNT(movie_id) AS movie_count
from ratings as r
join movie as m
on r.movie_id = m.id
where date_published between '2018-04-01' and '2019-04-01'  and median_rating=8 group by median_rating ;
...
group by languages order by total_votes ;
...
select production_company, sum(total_votes) as vote_count,
	...
	from movie as m
      join ratings as r
    on m.id = r.movie_id group by production_company limit 3 ;
